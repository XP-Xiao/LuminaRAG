...
class DocumentLoader:
    ...
    def load_document(self, file_path: str, filename: str) -> list[dict]:
        ...
        if file_lower.endswith(".pdf"):
            doc_type = "PDF"
            try:
                from backend.indexing.pdf_processor import load_pdf_for_document_loader

                raw_docs = load_pdf_for_document_loader(file_path, filename)
                return self._load_from_langchain_docs(raw_docs, file_path, filename, doc_type)
            except Exception:
                # MinerU 不可用/解析失败时降级回 PyPDFLoader 文本层提取
                loader = PyPDFLoader(file_path)
        elif file_lower.endswith(".docx"):
            doc_type = "Word"
            from backend.indexing.docx_processor import load_docx_for_document_loader

            raw_docs = load_docx_for_document_loader(file_path, filename)
            return self._load_from_langchain_docs(raw_docs, file_path, filename, doc_type)
        elif file_lower.endswith(".xlsx"):
            doc_type = "Excel"
            from backend.indexing.xlsx_processor import load_xlsx_for_document_loader

            raw_docs = load_xlsx_for_document_loader(file_path, filename)
            return self._load_from_langchain_docs(raw_docs, file_path, filename, doc_type)
